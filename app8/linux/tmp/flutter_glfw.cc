...
static void GLFWMouseButtonCallback(GLFWwindow* window,
                                    ...
                                    int action,
                                    int mods) {
  std::cerr << "[" << getTimeStamp() << "]" << "[GLFWMouseButtonCallback]" << " key " << action << " action " << " mods " << mods << std::endl;
...
static void GLFWScrollCallback(GLFWwindow* window,
                               double delta_x,
                               double delta_y) {
  std::cerr << "[" << getTimeStamp() << "]" << "[GLFWScrollCallback]" << " delta_x " << delta_x << " delta_y " << std::endl;
  FlutterPointerEvent event = {};
  SetEventLocationFromCursorPosition(window, &event);
  SetEventPhaseFromCursorButtonState(window, &event);
...
static void GLFWCharCallback(GLFWwindow* window, unsigned int code_point) {
  std::cerr << "[" << getTimeStamp() << "]" << "[GLFWCharCallback]" << " code_point " << code_point << std::endl;
...
static void GLFWKeyCallback(GLFWwindow* window,
  ...
  std::cerr << "[" << getTimeStamp() << "]" << "[GLFWKeyCallback]" << " key " << key << " scancode " << scancode << " action " << action << " mods " << mods << std::endl;
...
// GLFWKeyCallback  键盘输入
// GLFWCharCallback 输入字符
// GLFWMouseButtonCallback // 鼠标事件
// GLFWScrollCallback // 滚动
static void GLFWAssignEventCallbacks(GLFWwindow* window) {
  glfwPollEvents();
  // 处理特定的键盘key https://www.glfw.org/docs/latest/group__input.html#ga1caf18159767e761185e49a3be019f8d 
  // Keyboard keys https://www.glfw.org/docs/latest/group__keys.html
  glfwSetKeyCallback(window, GLFWKeyCallback);
  // This function sets the character callback of the specified window, which is called when a Unicode character is input.
  // https://www.glfw.org/docs/latest/group__input.html#gab25c4a220fd8f5717718dbc487828996
  glfwSetCharCallback(window, GLFWCharCallback);
  // This function sets the mouse button callback of the specified window, which is called when a mouse button is pressed or released.
  // https://www.glfw.org/docs/latest/group__input.html#ga6ab84420974d812bee700e45284a723c
